...
\begin{document}
    ...
    * podkreślić właściwe
    ...
    wymagałoby \begin{math}
                   9x9=81
    \end{math} operacji. Natomiast przy użyciu obrazu scałkowanego liczba operacji jest
    stała i wynosi dokładnie 4 \cite{violaJones}.

    Liczba cech wykrywanych w zdjęciu za
    pomocą filtru Haara wynosi zwykle powyżej stu tysięcy \cite{violaJones}. Wiele z nich nie ma
    większego znaczenia w procesie wykrywania twarzy. Natomiast przy tak dużej ilości cech na obrazie potrzebna jest dodatkowa
    technika, która przyspieszy detekcję twarzy. Taką właśnie techniką jest AdaBoost. Została ona opracowana
    ...
    wytworzenia silnych klasyfikatorów. Silny klasyfikator powstaje jako liniowa kombinacja słabych
    ...
    Biblioteka OpenCV zawiera wytrenowane klasyfikatory, które zostały użyte w tej pracy magisterskiej. Wykorzystano
    je do wykrycia twarzy, ust oraz oczu. Klasyfikatory mają postać plików xml, które można znaleźć na oryginalnym
