...
\begin{document}
    ...
    * podkreślić właściwe
    ...
    \begin{figure}
        \centering
        \includegraphics[width=8cm]{Obrazy/dobryRyjekWszystkieStrefyGray.png}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=8cm]{Obrazy/wrinkleAreas2.jpg}
        \caption{Przykład błędnej identyfikacji zmarszczek w strefie 2.}
        \label{fig.zlyRyjek}
    \end{figure}
    ...
    \begin{itemize}
        \item Dla HOG-a (parametry takie i taki)%todo
        \item Dla Canny
        %todo
    \end{itemize}

    Natomiast dla zdjęcia zawierającego nieprawidłowo wykryte krawędzie ilość zmarszczek wyniosła
    \begin{itemize}
        \item Dla HOG-a (parametry takie i taki)%todo
        \item Dla Canny
        %todo
    ...
    Kolejnym problemem okazała się grzywka nachodząca na obszar strefy 1. Na obrazie (ref) można zauważyć wykryte krawędzie,
    które identyfikują włosy. Obraz (ref) należy do x letniej osoby. Podobnie, jak powyżej, dokonano porównanie,
    które ma na celu sprawdzenie wpływu nieprawidłowo wykrytych krawędzi na współczynnik zmarszczek.
    Do powyższego celu obliczono średni współczynnik zmarszczek ze strefy 2 dla osób w wieku x lat, który wyniósł:
    ...
    Różnica pomiędzy średnią ilością zmarszczek w strefie 2 dla wieku x lat
    ,a ilością dla obrazu zawierającego nieprawidłowe wykryte krawędzie wynosi (Obraz \ref):
    \begin{itemize}
        \item Dla HOG-a (parametry takie i taki)%todo
        \item Dla Canny
        %todo
    ...
    Baza UTKface zawiera x zdjęć. Detektor twarzy był w stanie wyznaczyć obszar twarzy dla x obrazów.
    Jednak spośród x obrazów tylko x brało udział w liczeniu zmarszczek, ponieważ jedynie w tej grupie algorytm poprawnie wykrył
    ...
    Średni czas wykrywania twarzy trwał x sekund, maksymalny czas wykrywania wynosił x sekund, natomiast minimalny czas wyniósł
    x sekund.


    Średni czas generacji wrinkle feature obrazu po wykryciu twarzy wyniósł x sekund dla metody bazowej. Dla metody, w której została
    odjęta strefa x sekund, dla metody HOG x sekund, natomiast dla metody HOG wraz z klasyfikacją KNN x sekund.

    Maksymalny czas generacji wrinkle feature obrazu po wykryciu twarzy wyniósł x sekund dla metody bazowej. Dla metody, w której została
    odjęta strefa x sekund, dla metody HOG x sekund, natomiast dla metody HOG wraz z klasyfikacją KNN x sekund.

    Minimalny czas generacji wrinkle feature obrazu po wykryciu twarzy wyniósł x sekund dla metody bazowej. Dla metody, w której została
    odjęta strefa x sekund, dla metody HOG x sekund, natomiast dla metody HOG wraz z klasyfikacją KNN x sekund.
    ...
    Średni czas szacowania wieku dla danego obrazu po wykryciu twarzy wyniósł x sekund dla metody bazowej. Dla metody, w której została
    odjęta strefa x sekund, dla metody HOG x sekund, natomiast dla metody HOG wraz z klasyfikacją KNN x sekund.

    Maksymalny czas szacowania wieku dla danego obrazu po wykryciu twarzy wyniósł x sekund dla metody bazowej. Dla metody, w której została
    odjęta strefa x sekund, dla metody HOG x sekund, natomiast dla metody HOG wraz z klasyfikacją KNN x sekund.

    Minimalny czas szacowania wieku dla danego obrazu po wykryciu twarzy wyniósł x sekund dla metody bazowej. Dla metody, w której została
    odjęta strefa x sekund, dla metody HOG x sekund, natomiast dla metody HOG wraz z klasyfikacją KNN x sekund.

    Najszybciej szacował wiek algorytm x.
    Warto podkreślić, że szacowanie wieku dla metody HOG wraz z klasyfikacją KNN realizował program realizowany przez skrypt w Matlabie.
    Reszta algorytmów szacowania wieku została zrealizowana w programie w języku Java. W związku z powyższym porównanie szybkości działania
    ...
    $x$ procent np. 90\%, a zbiór testowy odpowiednio $100\% -x$, np 10\%.%cytat http://edu.pjwstk.edu.pl/wyklady/adn/scb/wyklad9/w9.htm
    ...
    Każda iteracja daje wynik działania klasyfikatora. W wyniku ,,n'' iteracji otrzymuje się uśredniony wynik działania klasyfikatora. %cytat http://edu.pjwstk.edu.pl/wyklady/adn/scb/wyklad9/w9.htm
    ...
    Metoda Leave-One-Out działa dokładnie tak samo jak metoda CV-n dla $n$ równego liczbie obiektów w zbiorze danych.%cytat http://edu.pjwstk.edu.pl/wyklady/adn/scb/wyklad9/w9.htm

    Przy obliczaniu skuteczności klasyfikacji porównywano parametr MAE Wzór \ref{wzor.mae}. Porównywano 100 zdjęć dla każdej metody.
    ...
    ulegały parametry wyżej wymienionego algorytmu. Analogicznie zmieniano parametry dla metody HOG i klasyfikatora KNN.
    ...
    Pierwsza została przetestowana metoda bazowa.
    Przetestowano dokładność szacowania wieku zmieniając parametry algorytmu trenującego Fuzzy C-means:

    rozmycia, \epsilon, ilości grup, liczby iteracji

    Przykładowy wynik testowania metody został przedstawiony na (obrazie?, tabeli).
    Dla parametrów rozmycie = 2.0, liczba iteracji = 100 $$ oraz liczba grup = 10 błąd MAE wyniósł 12,
    75 lat.
    Następnie zmieniono rozmycie do wartości 3.0. Jakość algorytmu szacującego wiek poprawiła się o 1,45 lat u wyniosła 11,3.
    Ostatnią testowana wartość parametru rozmycia wyniosła 5. W wyniku testu MAE wyniosło 11,67 lat.

    W kolejnym kroku przetestowano wpływ parametru \epsilon dla metody oryginalnej.
    Dla parametrów rozmycie = 2.0, liczba iteracji = 100 \epsilon = 1e-6 oraz liczba grup = 10 błąd MAE wyniósł 11,19 lat.
    Zmiana parametru \epsilon do 1e-7 spowodowała, że parametr MAE wyniósł 11,24 lat.
    Dla wartości 1e-8 parametru \epsilon parametr MAE również wyniósł 11,24 lat.
    Należy podkreślić, że liczba iteracji została zmieniona do 1000, ponieważ ograniczała ona osiągnięcie zadanej wartości \epsilon zarówno
    dla wartości \epsilon 1e-7 oraz 1e-8.
