...
\begin{document}
    ...
    * podkreślić właściwe
    ...
    Celem niniejszej pracy jest zbadanie pewnego algorytmu szacowania wieku.
    Odtworzono pracę ,,Age Estimation from Face Image using Wrinkle Features'' \cite{wrinkleFeatures}
    , a następnie zaimplementowano kilka modyfikacji powyższego algorytmu w celu polepszenia wyników.

    Zakres pracy jest następujący:
    \begin{itemize}
        \item Implementacja oryginalnej metody
        \item Zaproponowanie kilku modyfikacji powyższej metody
        \item Opis algorytmów oraz zmian w stosunku do oryginalnej metody
        \item Przeprowadzenie badań w celu roztrzygnięcia, który algorytm jest najlepszy
    ...
    Powyższe porównanie ma na celu sprawdzenie wpływu nieprawidłowo wykrytych krawędzi na współczynnik zmarszczek.
    \clearpage
    Średni współczynnik zmarszczek w strefie 2 dla osób w wieku 17 lat wyniosła:
    ...
    Obraz~\ref{fig.grzywka} należy do 51 letniej osoby. Podobnie, jak powyżej, dokonano porównanie,
    które ma na celu sprawdzenie wpływu nieprawidłowo wykrytych krawędzi na współczynnik zmarszczek.
    Do powyższego celu obliczono średni współczynnik zmarszczek ze strefy 2 dla osób w wieku 51 lat, który wyniósł:
    ...
    Natomiast dla zdjęcia zawierającego nieprawidłowo wykryte krawędzie (Obraz~\ref{fig.grzywka}) ilość zmarszczek wyniosła
    \begin{itemize}
    ...
    Średni czas wykrywania twarzy trwał 0,125 sekund, maksymalny czas wykrywania wynosił 0,456 sekund, natomiast
    minimalny czas wyniósł 0,056 sekund.

    Średni czas generacji wrinkle feature obrazu po wykryciu twarzy wyniósł 0,467 sekund dla metody bazowej.
    Dla metody, w której została odjęta strefa 0,465 sekund
    , dla metody HOG 0,543 sekund, natomiast dla metody HOG wraz z klasyfikacją KNN 0,543 sekund.

    Maksymalny czas generacji wrinkle feature obrazu po wykryciu twarzy wyniósł 0,723 sekund dla metody bazowej. Dla
    metody, w której została
    odjęta strefa 0,716 sekund, dla metody HOG 0,954 sekund
    , natomiast dla metody HOG wraz z klasyfikacją KNN 0,956 sekund.

    Minimalny czas generacji wrinkle feature obrazu po wykryciu twarzy wyniósł 0,286 sekund dla metody bazowej.
    Dla metody, w której została odjęta strefa 0,274 sekund
    , dla metody HOG 0,315 sekund, natomiast dla metody HOG wraz z klasyfikacją KNN 0,342 sekund.

    W związku z powyższym najszybciej generował wrinkle feature algorytm metody po odjęciu jednej strefy.

    Warto porównać szybkość szacowania wieku na podstawie danego wrinkle feature.

    Średni czas szacowania wieku dla danego obrazu wyniósł 0,585 sekund dla metody bazowej.
    Dla metody, w której została
    odjęta strefa 0,583 sekund, dla metody HOG 0,675 sekund, natomiast dla metody HOG wraz z klasyfikacją KNN 0,678
    sekund.

    Maksymalny czas szacowania wieku dla danego obrazu po wykryciu twarzy wyniósł 0,95 sekund dla metody bazowej.
    Dla metody, w której została
    odjęta strefa 0,947 sekund, dla metody HOG 1,152 sekund, natomiast dla metody HOG wraz z klasyfikacją KNN 1,15
    sekund.

    Minimalny czas szacowania wieku dla danego obrazu po wykryciu twarzy wyniósł 0,235 sekund dla metody bazowej. Dla
    metody, w której została
    odjęta strefa 0,23 sekund, dla metody HOG 0,426 sekund, natomiast dla metody HOG wraz z klasyfikacją KNN 0,415
    sekund.
    ...
    Reszta algorytmów szacowania wieku została zrealizowana w programie w języku Java.
    ...
    Natomiast każda metoda miała zmieniane parametry algorytmów. W przypadku metody z algorytmem grupowania Fuzzy C-means zmianie
    ...
    Dla parametrów m = 2,0, n = 100 , g = 10 oraz $\epsilon$=1e-5 błąd MAE wyniósł 12,75 lat.
    Następnie zmieniono m do wartości 3.0. Jakość algorytmu szacującego wiek poprawiła się o 1,45 lat
    i wyniosła 11,3.
    Ostatnia testowana wartość parametru m wyniosła 5. W wyniku testu MAE wyniosło 11,67 lat.
    ...
    Zmiana parametru $\epsilon$ do 1e-7 spowodowała, że parametr MAE wyniósł 11,24 roku.
    ...
    ograniczała osiągnięcie zadanej wartości $\epsilon$ zarówno
    dla wartości $\epsilon$ 1e-7, jak i dla 1e-8.

    Następnie testowano wpływ liczby grup. Dla m = 2,0, n = 100 , g = 20 oraz $\epsilon$=1e-5 MAE wyniósł 10,97 lat.
    W kolejnym kroku zmieniono liczbę grup do 50. Otrzymano MAE wyniosło 10,66 lat. W ostatnim kroku liczba grup
    wyniosła 100, natomiast parametr MAE był równy 10,68 lat.

    Minimalna wartość MAE dla metody bazowej wyniosła 10,66 lat.
    ...
    Powyższy błąd wyniósł 13,66 lat.
    Następnie zmieniono parametr m na wartość 3,0. Parametr MAE dla powyższej wartości wyniósł 13,74 lat.
    Ostatnią wartość m wyniosła 5,0. MAE wyniosła 14,41 lat.

    Następnie analizowano wpływ parametru $\epsilon$. Tak jak w testach z Sekcji \ref{subsec:metoda-bazowa} należało
    zmienić parametr n do 1000.
    Najpierw przetestowano MAE dla wartości
    $\epsilon$ = 1e-6.
    Wyniosło ono 13,65. Dla wartości $\epsilon$ = 1e-7 parametr MAE był równy 13,77. MAE było równe 12,31 lat dla
    $\epsilon$ = 1e-8.

    Na samym końcu przeanalizowano wpływ liczby klastrów na MAE. Najpierw zbadano wartość MAE dla 20 grup. MAE
    wyniosło 13,62. Następnie podwyższono wartość do g = 50. Dla powyższej wartości MAE wyniosło 13,61 lat.
    Ostatnią wartością było g = 100 lat. MAE wyniosło 13,74 lat.

    Minimalna wartość MAE dla opisywanej modyfikacji wyniosła 12,31 lat.
    Została ona osiągnięta dla m = 2,0, g = 10, $\epsilon$ = 1e-8, n = 1000

    \subsection{Metoda HOG}\label{subsec:metoda-hog}
    Kolejną przetestowaną modyfikacją była, ta w której użyto algorytm HOG.
    Podobnie jak w Sekcjach \ref{subsec:odjęcie-strefy-2} oraz \ref{subsec:metoda-bazowa} modyfikowano parametry
    algorytmu Fuzzy C-means. Jednak dodatkowo modyfikowano parametry HOG. Powyższym parametrem był rozmiar komórki.
    Najpierw ustalono wymiary komórki 3x3 pikseli, rozmiar okna wyniósł 6x6 pikseli, natomiast liczba komórek
    histogramu była stała i wynosiła 9 dla wszystkich pomiarów. Dla komórki 6x6 piksela rozmiar okna wynosił 12x12
    piksela, natomiast rozmiar okna wynosił 18x18 piksela dla komórki równej 9x9 piksela.

    W pierwszym teście ustalono parametry Fuzzy C-means: m = 2,0, n = 100 oraz g = 10, n = 100 oraz $\epsilon$ = 1e-5.
    Wartość MAE wyniosła 16,57. Dla wartości komórki równej 6x6 pikseli wartość MAE wyniosła 16,44. Natomiast dla
    ...
    Przy zmianie parametru m do 3,0 wartość MAE wyniosła 16,82 lat dla komórki 3x3 piksela, natomiast 16,73 lat dla
    komórki 6x6 lat. Dla komórki 3x3 pikseli, zmiana m do
    5,0 zmniejszyła MAE do 16,79 lat, natomiast komórka 6x6 piksela zwiększyła MAE do 16,89 lat.
    Dla parametrów m = 2,0 n = 1000, $\epsilon$ 1e-6, g = 10 oraz komórki 3x3 piksela MAE wyniosło 16,44 lat,
    natomiast przy zmianie komórki do rozmiarów 6x6 piksela MAE wyniosło 16,44 lat.
    Dla zmiany parametru $\epsilon$ do 1e-7, MAE wyniosło 16,76 lat przy komórce o wymiarach 6x6 piksela, dla komórki
    3x3 piksela, MAE wyniosło 16,49 lat.
    Dla wartości $\epsilon$ = 1e-8 MAE osiągneło 16,44 lat przy rozmiarach 6x6 piksela, natomiast dla wymiarów 3x3
    piksela wartość MAE wyniosła 16,7 lat.

    Dla parametrów m = 2,0, n = 100 oraz g = 20, n = 100, $\epsilon$ = 1e-5 oraz rozmiaru komórki równej 3x3 piksela
    MAE było równe 16,49 lat, natomiast dla rozmiarów 6x6 piksela, MAE wyniosło 16,77 lat.
    Przy zmianie g do 50 MAE zmieniło wartość do 16,16 lat, dla komórki 6x6 piksela, przy zmianie rozmiaru komórki do
    3x3 piksela MAE było równe 15,89 lat. Dla
    parametru g =
    100 MAE oraz komórki 6x6 piksela
    wyniosło 16,5 lat, natomiast dla komórki o wymiarach 3x3 piksela MAE było równe 15,45 lat.

    Minimalna wartość MAE dla tej metody wyniosła 11,3 lat. Jak wyżej wspomniano, wartość nie była zależna od
    parametrów algorytmu Fuzzy C-means.

    \subsection{Metoda HOG + KNN}\label{subsec:metoda-hog-+-knn}

    Ostatnią testowaną metodą była ta, która zawiera algorytm HOG oraz do klasyfikacji i szacowania używa algorytmu KNN.
    Zmieniano parametr n dla algorytmu KNN, natomiast dla algorytmu HOG-a rozmiary komórki.
    Parametr n był równe 1, 3 oraz 5. Rozmiary komórki miały wartość 5x5 piksela, 7x7 piksela oraz 9x9 piksela.
    Liczna komórek histogramu była stała i wynosiła 9. Podobnie jak w Sekcji \ref{subsec:metoda-hog} zmieniano
    rozmiary bloku. Dla rozmiaru komórki 5x5 piksela, blok miał rozmiar 10x10 piksela. Blok o wymiarach 14x14 piksela
    był testowany wraz z wartością komórki 7x7 piksela. Ostatni rozmiar komórki 9x9 piksela był ustawiany wraz z
    rozmiarem bloku 18x18 piksela.

    Przy parametrze n = 1 oraz wymiarach komórki 5x5 MAE wyniosło 11,75 lat. Przy zmianie parametru do n = 3 MAE
    zmniejszyło się do 11,65 lat. Dla ostatniej wartości knn = 5 MAE było najmniejsze i było równe 11,38 lat.

    Następnym krokiem było badanie jakości szacowania wieku dla wartości komórki 7x7 piksela.
    MAE wyniosło 10,77 lat dla n = 1, 10,34 lat dla n = 3 oraz 10,56 lat dla n = 5.

    Ostatnią badanym rozmiar komórki 9x9 piksela przyniósł następujący rezultat.
    Dla n = 1 MAE wyniosło 10,15 lat. Zmiana parametru n do 3 zmieniła MAE do wartości 10,07 lat.
    Dla ostatniej wartości n równej 5 MAE osiągnęło najmniejszą wartość w całym procesie testowania - 9,97 lat.
    ...
    jednej próby. Przy parametrach m = 2.0, n = 100 $\epsilon$ = 1e-5 oraz g = 10 MAE wyniosło 12,75 lat. Dla reszty
    parametrów zmiany MAE były w zakresie od 10,66 lat do 11,67 lat. Największe polepszenie wskaźnika MAE uzyskano
    przy zmianie parametru g na 20 i więcej. Przy powyższej zmianie wartość MAE spadła poniżej 11 lat.

    Dla modyfikacji, która polegała na odjęciu strefy nr 2 wyniki znacznie sie pogorszyły. Modyfikacja parametru m
    pogarszała wskaźnik MAE od wartości 13,66 do 14,41 lat. Minimalna wartość MAE została osiągnięta dla parametrów
    m = 2.0, n = 1000 $\epsilon$ = 1e-8 oraz g = 10. Warto zaznaczyć, że modyfikację liczbą klastrów nie polepszyła
    MAE tak jak w przypadku testowania metody bazowej.

    W metodzie zawierającą modyfikację z algorytmem HOG przetestowano 27 kombinacji parametrów - 3 razy więcej niż przy
    testowaniu metody bazowej. W porównaniu do metody z odjętą strefą MAE uległo znacznemu pogorszeniu. Dla komórki o
    rozmiarach 3x3 piksela parametry Fuzzy C-means nie dawały znacznej
    poprawy rezultatów MAE. Wskaźnik MAE osiągał wartości od 15,45 do 16,82 lat. Podobne wartości były osiągane dla
    komórki o rozmiarach 6x6 piksela. W tym przypadku MAE było w zakresie od 16,16 do 16,89 lat.

    Ostatnia modyfikacja przyniosła poprawę w porównaniu do metody bazowej.
    Dla rozmiaru komórki 5x5 piksela oraz dla zmiennego parametru n algorytmu KNN, MAE osiągało wartości od 11,38 do
    11,75 lat. Warto zauważyć, że poprawa wskaźnika MAE następowała wraz ze wzrostem parametru n. Zmiana rozmiaru
    komórki na 7x7 polepszyła wskaźnik MAE. Osiągał on wartości od 10,34 do 10,77 lat. W tym przypadku najlepszą
    wartość MAE osiągnięto dla wartości n = 3, czyli wartości pośredniej, z pośród wszystkich przetestowanych.
    Najlepsze wartości osiągnięto dla komórki o rozmiarach 9x9. Minimalną i zarazem najlepszą dla wszystkich testów
    ...
    to wartość zadawalająca, gdyż w literaturze powyższy wskaźnik osiągał wartości około 5 lat. \cite{khryashchevGanin}
    Udało się polepszyć algorytm bazowy. Modyfikacja zawierająca algorytm HOG oraz KNN polepszyła MAE z 10,66 lat do
    9,97 lat. W celu poprawy wyników należałoby zwiększyć bazę treningową. Z ponad 20000 obrazów z bazy tylko 2574
    brało udział w treningu. Wiele zdjęć było odrzuconych z powodu braku detekcji przez detektor oczu oraz nosa. W
    związku z powyższym należałoby użyć lepszych, wyżej wymienionych detektorów lub znaleźć bazę, która zawiera
