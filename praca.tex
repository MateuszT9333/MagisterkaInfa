...
\begin{document}
    ...
    * podkreślić właściwe
    ...
    modyfikacji
    pod kątem skuteczności szacowania wieku. %todo zweryfikowac i napisac wstep
    ...
    %todo sprawdzic czy wszedzie ilosc iteracji jest rowna 1000
    ...
    Pozostałe parametry algorytmu nie podlegały zmianom.
    \begin{table}[h!]
        \label{tab.bazowa_m}
        \caption{MAE dla zmiany parametru m w metodzie bazowej.}
        \centering
        \begin{tabular}{|M{1cm}|M{1cm}|M{1cm}|M{1cm}|}
            & m=2,0 & m=3,0 & m=5,0 \\
            MAE & 12,79 roku & 11,25 roku & 11,68 roku
        \end{tabular}
    \end{table}
    Dla parametru $m=2,0$ MAE wyniosło 12,79 roku. Zmiana powyższego parametru do 3,0 zmniejszyła MAE do 11,25 roku,
    $m=5,0$ poprawił wynik do 11,68 roku. Najlepszy wynik dla modyfikacji parametru m, osiągnięto dla m=3,0.

    W kolejnym kroku przetestowano wpływ parametru $\epsilon$ dla metody oryginalnej.
    W tabeli przedstawiono, jak ten parametr wpływał na MAE.
    ...
    Następnie testowano wpływ liczby grup na parametr MAE.
    Kolejna tabela zawiera wyniki wyżej wymienionego testowania.
    %    Dla m = 2,0, n = 100 , g = 20 oraz $\epsilon$=1e-5 błąd
    %    MAE wyniósł 10,97 roku.
    ...
    \subsection*{Wyniki z walidacji krzyżowej CV-5} %todo
