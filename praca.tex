...
\begin{document}
    ...
    \includepdf[page=-]{wyrazaniezgody.pdf}

    %%%%%%%%%%%%%%%%%%%%% oswiadczenie promotora o spełnieniu wymagań formalnych %%%%%%%%%%%%%%%%%%%
    \cleardoublepage

    \includepdf[page=-]{oswiadczenie.pdf}
    ...
    Mogą to być na przykład osoby, pojazdy, przedmioty itp.
    ...
    opiera się w głównej mierze na sztucznej inteligencji.
    %275 wyrazow
    ...
    Features''. Metodę w niej zawartą poddano następnie kilku modyfikacjom.

    Zakres pracy obejmuje następujące zagadnienia:
    \begin{itemize}
        \item Implementacja algorytmu z pracy, w dalszej części pracy określanego jako metoda
    ...
    Badania tego rodzaju opisano w~pracy A. Lanitis, Ch. J. Taylor oraz T. F. Cootes.
    ...
    Taka analiza została zaprezentowana w pracy N. Ramanathan oraz R. Chellappa.
    Rozwinięciem tego pomysłu była praca X. Geng, Z. Zhou i K. Smith-Miles.
    ...
    Z kolei M.M.~Dehshibi oraz A.~Bastanfard w swojej pracy analizują proporcje twarzy oraz ilość
    ...
    Powyższa metoda została zaprezentowana w książce Guodong Guo i in.
    Mniej popularne metody analizujące cechy twarzy to filtry Gabora oraz LBP (ang. local binary patterns).


    W metodzie bazowej, która została opisana w artykule, wykrywanie wieku dzieli się na kilka
    faz.
    ...
    Należy jednak zauważyć, że w procesie ekstrakcji twarzy z obrazu mogą wystąpić liczne problemy.
...
%    " jako ostre jak przedstawiono na rys. 2.3." Proszę zobaczyć czy w innych miejscach też nie da się w prosty sposób wprowadzić odwołania do rynku jako części zdania.
    ...
    %https://sci-hub.tw/10.1007/s10462-018-9650-2
    W roku 1997 w pracy opisano metodę wykrywania twarzy bazującą na
    ...
    Algorytm został bliżej przedstawiony w książce w 2001 roku i składa się z trzech podstawowych faz,
    ...
    Najczęściej stosowaną barwą w takich obrazach jest szarość.

    Istnieją 3 metody konwersji obrazu z przestrzeni RGB na monochromatyczny: %inne cytowanie tutaj dac
    \begin{itemize}
    ...
    Został stworzony przez Paula Violę oraz Michaela Jonesa w 2001 roku.
    ...
    \end{equation}
    \normalsize
    gdzie $I(x,y)$ jest wartością na pozycji $x$, $y$ w tabeli(tabela obrazu scałkowanego), natomiast $i(x,y)$ oznacza
    jasność piksela o
    ...
    na rysunku~\ref{fig.sumowanieOkna}, $I(A)$ - wartość scałkowanego obrazu przy punkcie A~(analogicznie $I(B)$, $I(C)$, $I(D)$) - (rysunek~\ref{fig.sumowanieOkna}).
    ...
    pomocą filtru Haara jest znacznie większa od liczby pikseli na obrazie.
    ...
    2. Dla każdej cechy $j$, trenowany jest klasyfikator $h_{j}$, który używa tylko jednej cechy wyliczonej z filtru
    Haara.
    ...
    Pierwsze okna posiadają klasyfikatory o słabszym TPR (ang. true positive rate) oraz FPR (ang. false positive rate)
    niż kolejne.
    ...
    Ostatnie okna mają największy współczynnik TPR oraz najmniejszy FPR ze wszystkich.
    ...
    To właśnie te miejsca zostały uznane przez autorów książki za
    ...
    Autorzy algorytmu założyli, że odległość od linii oczu do linii brwi wynosi $0,4 \cdot d$,
    ...
    Algorytm detektora jest podzielony na kilka kroków:
    \begin{itemize}
    ...
    Rozmiar filtra ma wpływ na wydajność wygładzania obrazu, a co za tym idzie na czułość wykrywania szumu.
    ...
    W wyniku tego działania otrzymuje się obraz gradientowy.
    ...
    to dany piksel należy do najostrzejszej krawędzi.
    ...
    Krawędź oznacza się jako mocną, gdy wartość gradientu dla niej jest większa od wartości progu dużego.
    ...
    W przypadku, gdy słaba krawędź nie jest powiązana z żadną mocną krawędzią - zostaje usunięta.
    ...
    Zdjęcia są różnej jakości i nie każda baza zdjęć nadaje się do konkretnych badań.
    ...
    można
    oszacować wiek. %todo sprawdzic referencje na sam koniec ctrl + f i "?" w Adobe reader
    ...
    Powyższe grupy zawierają dane, które są podobne do siebie.
    ...
    C-means.
    Należy ona do grupy modeli centroidowych. W Fuzzy C-means zastosowano klasteryzacje miękką.
    ...
    a następnie zostaje wyliczony parametr wskaźnik zmarszczek $WF_{i}$.
    ...
    \end{equation}
    \normalsize
    gdzie $\left \|v  \right \|_{2}$ jest wartością $L2$ dla wektora $v$. Działanie zgodne ze
    ...
    \right \|_{1}$  jest parametrem $L1$ dla wektora $v_{3}$.
    ...
    Dla parametru $k$ równego 3 wektor testowy zostanie przypisany do klasy $B$, gdyż z trzech najbliższych wektorów
    dwa
    ...
    W~wyniku n iteracji otrzymuje się uśredniony wynik działania klasyfikatora.
    ...
    próbki 10\% zdjęć z całego zbioru.
    ...
    \subsection*{Wyniki z testowania 10\% zdjęć}
    W pierwszym teście ustalono parametry Fuzzy C-means: $m$ = 2,0, $n$ = 100 oraz $g$ = 10, $g$ = 100
    oraz $\epsilon$ = 1e-5.
    ...
    Aby osiągnąć założony cel, dokonano implementacji oryginalnego algorytmu,
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %todo w bibliografii tylko inicjaly imiona
    \backmatter
    \pagenumbering{Roman}
    ...
    \begin{appendices}
        ...
        \begin{itemize}
            ...
            \item[TPR] - \ang{ang. true positive rate} - współczynnik prawidłowej klasyfikacji jako obiekt.
            \item[FPR] -  \ang{ang. false positive rate} - współczynnik nieprawidłowej klasyfikacji jako obiekt.
        \end{itemize}
