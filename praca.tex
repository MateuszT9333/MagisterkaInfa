...
\begin{document}
    ...
    * podkreślić właściwe
    ...
    wykrywa także włosy. Przeglądając wiele zdjęć zauważono, że w obrębie
    strefy zmarszczek nr 2 (Obraz \ref{fig.wykrywanieStrefZmarszczkowych}) wykrywane są części brwi oprócz samych
    zmarszczek. Generuje to błąd, dlatego wyżej wymienionej strefy nie uwzględniano w obliczeniach.
    W Sekcji \ref{subsec:zmiana-algorytmu-względem-metody-bazowej} porównano różnice w algorytmie przed wyżej
    wymienioną modyfikacją i po modyfikacji.
    ...
    Po modyfikacji parametr wrinkle feature jest obliczany według Wzoru \ref{wzor.wrinkleFeature2}.
    \large
    \begin{equation}
    ...
    \end{equation}
    \normalsize
    Kolejną modyfikacją było zastosowanie metody HOG - Histograms of Oriented Gradients, która została opisana w Sekcji
    \ref{sec:zastosowanie-metody-hog}.
    ...
    Deskryptor cech wyodrębnia pewne cechy z obrazu.
    Do takich cech należą między innymi kształty, kolory, tekstury.
    Podobnie tak jak w metodzie bazowej taką rolę spełnia detektor
    krawędzi Canny (Sekcja \ref{sec:wykrywanie-zmarszczek---detektor-canny}), który wykrywa krawędzie,
    które identyfikują zmarszczki.
    Histogram of Oriented Gradients jest techniką, która liczy wystąpienia kierunków gradientów jasności.
    Liczenie jest dokonywane w pewnych obszarach obrazów.
    ...
    Jednak metoda została pierwszy raz zaimplementowana w 2005 roku przez Navneeta Dalala i Billa Triggsa \cite{hogZabojady}
    z francuskiego instytutu zajmującego się badaniami w zakresie informatyki i automatyki
    (INRIA - Institut national de recherche en informatique et en automatique).
    Powyższa metoda została zastosowana do detekcji pieszych na zdjęciach.
    Później metoda została rozszerzona o wykrywanie ludzi w filmach wideo.
    Ponadto zastosowano ją do wykrywania zwierząt oraz pojazdów na zdjęciach.

    Metoda Histogram of Oriented Gradients opiera się na założeniu, że wygląd i kształt obiektów na obrazie może zostać
    opisany przez gradient jasności oraz jego kierunek. Dobrze to jest widoczne na
    ...
    W ten sposób zostaję zidentyfikowana krawędź głowy.

    Algorytm działa w skrócie w następujący sposób. Obraz jest dzielony na małe obszary zwane komórkami.
    Zwykle obszary mają wymiar kilka na kilka pikseli.
    Dla każdego piksela w komórce tworzony jest histogram kierunków gradientu.
    Histogramy są łączone w jeden wspólny deskryptor.
    Dla zwiększenia dokładności lokalne histogramy normalizowane są pod względem kontrastu.
    Realizowane jest to przez pomiar jasności obszaru zwanym blokiem.
    Blok składa się z kilku komórek. Normalizacja zapewnia zmniejszenia niepożądanego efektu generowanego przez
    różnice w oświetleniu różnych obszarów zdjęcia. Powyższa sytuacja występują na przykład, gdy zdjęcie wykonywane
    jest w nieprawidłowych warunkach oświetleniowych (Rysunek \ref{fig.oswietlenieTwarzy}).

    W tym akapicie zostanie przedstawiony dokładny opis metody Histogram of Oriented Gradients.
    W pierwszej fazie obraz obraz powinien być przeskalowany, tak aby jego wymiary były podzielne przez rozmiar
    pojedyńczej komórki.
    ...
    \end{figure}
    Warto dodać, że w wielu metodach generujących deskryptory pierwszą fazą jest korekcja luminacji.
    Jednak ten krok może zostać pominięty, gdyż w późniejszym kroku stosowana jest normalizacja,
    która dokonuje korekcję luminacji cieni oraz świateł.
    W kolejnej fazie muszą zostać wyznaczone gradienty jasności.
    Wyznaczenie gradientów jasności zostało opisane w Sekcji \ref{subsec:szukanie-gradientów-jasności}.
    W kolejnej fazie zostają wyznaczone histogramy gradientów dla każdej komórki.
    Na Rysunku \ref{fig.gradientJasnosciNaGlowie} został przedstawiony wynik działania poprzedniego kroku.
    Zostają wygenerowane dwie macierze. Pierwsza zawiera informację o gradiencie jasności każdego piksela komórki.
    Natomiast druga zawiera informację o kierunku każdego gradientu.
    Wartości kierunku gradientu wyrażone są w stopniach. W metodzie Histogram of Oriented Gradients
    zastosowano kierunki gradientu bez znaku. Oznacza to, że zakres kierunków jest od $0^{\circ}$ do $180^{\circ}$.
    ...
    Wartość gradientu równa 4 zostaje rozdzielona na dwie proporcjonalne wartości: 2 oraz 2, które zostają dodane do
    ...
    \end{equation}
    \normalsize
    , gdzie $X_{a}$ oznacza wartość przydzielaną do komórki reprezentujący mniejszy kąt (tak jak $0^{\circ}$
    dla powyższego przykładu), $A_{x}$ oznacza wartość kąta gradientu dla danego piksela (tak jak $10^{\circ}$
    dla powyższego przykładu), $A_{a}$ oznacza kąt dla komórki reprezentujący mniejszy kąt (tak jak $0^{\circ}$
    dla powyższego przykładu), $A_{b}$ oznacza kąt dla komórki reprezentujący większy kąt (tak jak $20^{\circ}$
    ...
    Powyższa procedura jest powtarzana. Kolejne wartości gradientów są dodawane do aktualnych wartości przechowywanych
    ...
    Kolejnym krokiem jest normalizacja bloków. Blok składa się z komórek. Zwykle blok ma wymiary 2x2 komórki.
    ...
    deskryptora dla danego bloku. Powyższy wektor składa się z wartości zawartych komórkach histogramu. Jak wiadomo z
    powyższego opisu, każdy histogram opisuje jedną komórkę, a blok zawiera n komórek.
    ...
    Normalizacja jest realizowana stosując Wzór
    \large
    \begin{equation}
    ...
    $\left \|v  \right\|_{2}$.
    Powyższy proces jest powtarzany dla wszystkich bloków.
    ...
    Metoda Histogram of Oriented Gradients jest odporna na działanie przekształceń geometrycznych
    ...
    W związku z powyższym opisywana metoda nadaje sie do detekcji ludzi na obrazach.

    W pracy opisywana metoda posłużyła do wykrywania krawędzi, które informują o zmarszczkach. Dokładny sposób
    zaimplementowania w pracy został opisany w Sekcji \ref{subsec:zastosowanie-w-projekcie2}
    ...
    Powyższy deskryptor posłużył do wykrycia ilości zmarszczek w strefach zmarszczkowych. Jak wiadomo z opisu w
    ...
    Sekcji \ref{sec:wyliczanieWrinkleFeature} krawędzie mogą zostać zinterpretowane jako zmarszczka.
    ...
    W bibliotece OpenCv zaimplementowany jest algorytm HOG, który na podstawie pewnego obszaru obrazu lub całego
    obrazu generuje wektor reprezentujący deskryptor. Biblioteka umożliwia ustawienie różnych parametrów HOG-a.
