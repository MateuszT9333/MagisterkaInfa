...
\begin{document}
  ...
  Również szacowanie wieku dla metody stosującej algorytm KNN przeprowadzono za pomocą wyżej wymienionego
   ...
   Jak już wcześniej zostało wspomniane, najbardziej wrażliwe na brak generacji deskryptora są strefy 3
    i 6.
     W pierwszym kroku należało wydedukować, jaki obszar zajmuje w przybliżeniu strefa 3 i 6 w stosunku do obszaru twarzy.
    Na podstawie logów aplikacji wygenerowanych w trakcie detekcji stref dla zdjęć z bazy treningowej wyznaczono
    średni obszar strefy 3 i 6. Następnie porównano rozmiar obszaru twarzy w pikselach z rozmiarem strefy 3 i 6.
    ...
    obliczeniach stwierdzono, że strefa 3 i 6 zajmuje od 8\% do 15\% na szerokość obszaru twarzy oraz od 15\% do 25\%
    ...
    Okazało się, że minimalny obszar zajęty przez twarz wyniósł x \% na szerokość całego obrazu oraz y \% na długość.
    W świetle powyższych danych należy stwierdzić, że obszar twarzy jest x razy węższy oraz y razy krótszy od obszaru całego obrazu.

    Zgodnie z wynikami pierwszej fazy badań, obszar twarzy powinien być x razy szerszy od szerokości bloku, a jednocześnie q razy węższy od samego obrazu,
    więc szerokość obrazu powinna być większa od szerokości bloku o współczynnik w1.

    Wzor....

    Obliczenia dla współczynnika w2 są realizowane w podobny sposób. Obszar twarzy powinien być y razy dłuższy od długości bloku, a jednocześnie jest y razy krótszy od długości obrazu.
    W związku z powyższym długość obrazu powinna być większa od długości bloku o współczynnik w2, który wynosi:
    Wzor
