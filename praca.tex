...
\begin{document}
    ...
    * podkreślić właściwe
    ...
    Odtworzono pracę ,,Age Estimation from Face Image using Wrinkle Features''~\cite{wrinkleFeatures}, a następnie zaimplementowano kilka modyfikacji powyższego algorytmu w celu polepszenia wyników.
    ...
    from facial images'' analizują proporcję twarzy oraz ilość zmarszczek przy szacowaniu wieku~\cite{dehshibiBastard}.
    ...
    Dla strefy 5 wyznacza sie wysokość (wzór~\ref{wzor.punktHD}) oraz szerokość na podstawie wzoru~\ref{wzor.punktWD}.
    ...
    \end{equation}
    \normalsize
    W k-tym kroku algorytmu wyliczona jest macierz funkcji przynależności $U^{k} = {u_{ij}^{k}}$ (wzór~\ref{wzor
    .funkcjaPrzynaleznosci}).
    \large
    \begin{equation}
