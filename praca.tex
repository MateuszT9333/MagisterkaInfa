...
\begin{document}
    ...
    * podkreślić właściwe
    ...
    Rachunek wariacyjny pomaga znaleźć charakterystyczną funkcję.
    ...
    Najczęściej używane bazy to FG-NET, MORTH II.
    Zawierają one dziesiątki tysięcy zdjęć.
    Zdjęcia są różnej jakości i nie każda baza zdjęć nadaje się do konkretnych badań~\cite{khryashchevGanin}.
    Przy realizacji tej pracy użyto bazę UTKFace,
    ponieważ ścieżka pliku dla każdego obrazu zawiera informacje na temat rzeczywistego wieku.
    W związku z powyższym z każdego zdjęcia z tej bazy otrzymywano dwie informację - rzeczywisty wiek danej osoby oraz
    współczynnik zmarszczek.

    Autorzy algorytmu do trenowania użyli algorytm grupowania - Fuzzy C-Means.
    W wyniku działania algorytmu otrzymuje się dane dzięki, którym można na podstawie współczynnika zmarszczek można
    oszacować wiek~\cite{wrinkleFeatures}.
    \section{Grupowanie danych - Fuzzy C-Means}\label{sec:grupowanieDanych}
    %    Grupowanie danych zwane również klasteryzacją polega na podzieleniu dużego zbioru danych na grupy.
    %    Powyższe grupy posiadają dane, które są podobne do siebie.
    ...
    Grupowanie polega na podzieleniu dużego zbioru danych na grupy.
    Powyższe grupy posiadają dane, które są podobne do siebie~\cite{clusterWstep}.

    Klasteryzacja nie jest algorytmem, lecz zadaniem, które może zostać wykonane na wiele sposobów.
    Klasteryzacja jest iteracyjnym procesem odkrywania danych w celu odnalezieniu relacji pomiędzy nimi.
    Odpowiedni algorytm grupowania i ustawienia parametrów (w tym parametry,
    ...
    Odległość pomiędzy członkami grupy jest głównym parametrem klasteryzacji hierarchicznej.
    Istnieje wiele innych modeli grupowania.
    Jednym z nich jest centroidowy.
    Zakłada on, że każda grupa jest zdefiniowana przez jeden wektor, który posiada wartość średnią.
    Istnieją też modele, które opierają sie na sieciach neuronowych.
    Zakładają one, że dane grupują się za pomocą nienadzorowanych sieci neuronowych.
    Spotykany jest także model oparty o rozkłady statystyczne.

    Klasteryzacja może być twarda lub miękka.
    Twarda klasteryzacja oznacza, że każdy obiekt może należeć tylko do jednej grupy.
    Natomiast miękka klasteryzacja oznacza, że każdy obiekt może w pewnym stopniu należeć do każdej z grup.
    To oznacza, że dany obiekt może na przykład w 10\% należeć do grupy x, a w 30\% do grupy y itd.
    Istnieje wiele innych rodzajów modeli grupowania.
